...
const MIN_CHANNEL_FUNDING_SHANNONS = 200_00000000n; // 200 CKB
...
let _peerPubkey: string | null = null;
let _fiberAvailable: boolean | null = null;

async function fiberRpc(url: string, method: string, params: unknown[] = []): Promise<unknown> {
  const response = await axios.post(url, {
    jsonrpc: "2.0", id: Date.now(), method, params
  }, { timeout: 5000 });
  if (response.data.error) throw new Error(`Fiber RPC error: ${JSON.stringify(response.data.error)}`);
  return response.data.result;
}

export async function checkFiberAvailable(rpcUrl: string): Promise<boolean> {
  if (_fiberAvailable !== null) return _fiberAvailable;
  try {
...
export async function getFiberStatus(rpcUrl: string): Promise<FiberStatus> {
  const available = await checkFiberAvailable(rpcUrl);
  if (!available) return { available: false, error: "Fiber node not running" };
  try {
    const nodeInfo = await fiberRpc(rpcUrl, "get_node_info") as { node_id: string };
    const channels = await fiberRpc(rpcUrl, "list_channels", [{}]) as { channels: Array<{ channel_id: string; state: { state_name: string }; local_balance: string }> };
...
async function getPeerPubkey(rpcUrl: string): Promise<string | null> {
  try {
    const result = await fiberRpc(rpcUrl, "list_peers", [{}]) as {
      peers: Array<{ node_id: string; addresses: string[] }>;
    };
    const peer = result.peers?.find(p =>
      p.addresses?.some(a => a.includes(FIBER_TESTNET_PEER_ID))
    );
...
async function connectToPeer(rpcUrl: string): Promise<boolean> {
  try {
    ...
    if (errMsg.includes("already") || errMsg.includes("exists")) return true;
    console.error(`[FIBER] Failed to connect to peer:`, err);
    return false;
  }
}

export async function ensureChannelOpen(rpcUrl: string): Promise<string | null> {
...
export async function sendFiberPayment(rpcUrl: string, amountShannons: bigint, description: string): Promise<boolean> {
  const available = await checkFiberAvailable(rpcUrl);
  if (!available) return false;
  const channelId = await ensureChannelOpen(rpcUrl);
  if (!channelId || !_peerPubkey) return false;
  try {
    ...
    }]) as { payment_hash: string; status: string };
...
export async function printFiberStatus(rpcUrl: string): Promise<void> {
